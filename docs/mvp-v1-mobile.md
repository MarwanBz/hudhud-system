...
- Offline submission stays local until backend validation, system credit check, and fee reservation succeed during sync.
...
**Acceptance signal**
- Agent can create a shipment while offline and receive an official tracking number only after sync succeeds with backend credit check and fee reservation.
...
**Acceptance signal**
- No two synced shipments receive the same official tracking number, and failed insufficient-credit shipments keep only their local temporary receipt ID.
...
**Acceptance signal**
- A newly created offline shipment appears immediately in the list as pending sync, and an insufficient-credit failure remains visible until it syncs successfully or the agent resolves it.
...
- Driver can open shipment details:
  ...
  - Full address.
  - Tracking number.
  - Payment/value details if needed by operation.
  - Notes.
...
**Acceptance signal**
- Driver can call the receiver from a shipment detail screen.
...
**Acceptance signal**
- A driver outcome creates a shipment event, updates shipment status, and appears in admin and tracking views after sync.
